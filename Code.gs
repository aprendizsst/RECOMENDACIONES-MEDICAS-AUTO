...
function reserveConsecutives_(user, payload) {
  ...
  try {
    ...
    const info = locateConsecutiveSheet_(ss);
    const year = new Date().getFullYear();
    let scan = scanConsecutives_(info, year, cfg.prefix, '');
    let max = scan.max; let maxRaw = scan.maxRaw || '';
    const result = [];
    for (let i=0;i<items.length;i++) {
      ...
      let existing = '';
      if (documentKey) {
        const docScan = scanConsecutives_(info, year, cfg.prefix, documentKey);
        existing = docScan.existing || ledgerLookup_(documentKey, ss.getId(), info.sheet.getName());
        if (existing && !consecutiveExists_(info, existing)) existing = '';
      }
      if (existing) {
    ...
    SpreadsheetApp.flush();
    return { items:result, source:'Google Sheets', sheetName:info.sheet.getName() };
  } finally { lock.releaseLock(); }
}
