(() => {
  ...
  async function refreshBackendDiagnostics() {
    ...
    try {
      ...
      $('consecutiveStatusDetail').textContent = `${status.spreadsheetName} · ${status.sheetName} · ${status.rowsRead ?? 0} consecutivo(s) válidos leídos · actual: ${status.currentDisplay ?? status.current ?? 0} · siguiente: ${status.nextDisplay ?? status.next}`;
  ...
  async function saveConsecutiveSettings() {
    ...
    try {
      ...
      $('consecutiveStatusBadge').className = 'status-badge success';
      $('consecutiveStatusDetail').textContent = `${result.spreadsheetName} · ${result.sheetName} · ${result.rowsRead ?? 0} consecutivo(s) válidos leídos · actual: ${result.currentDisplay ?? result.current ?? 0} · siguiente: ${result.nextDisplay ?? result.next}`;
