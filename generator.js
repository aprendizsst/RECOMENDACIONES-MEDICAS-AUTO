(() => {
  const OUTPUT_FIELDS = ['nombre','cargo','tipo_examen','identificacion','examenes_lista','estado_por_examen','recomendaciones_por_examen','recomendaciones_lista','recomendaciones_pendientes_revision','restricciones_lista','observaciones','remisiones','vigilancia_programa','lugar','zona','fecha'];
  function outputExamType(value) {
    const canonical = SSTProfiles?.canonicalExamType?.(value) || '';
  ...
  class GeneratorService {
    ...
    async fingerprint(data, format, assets) {
      ...
      body.documentEngineVersion = SSTDocx.engineVersion || 'template-engine-v10.26';
    ...
    async generatePdf(data, signatureAsset) {
      ...
      doc.setFillColor(239,246,255); doc.setDrawColor(207,226,250); doc.roundedRect(left, y, maxW, 11, 1.5, 1.5, 'FD'); doc.setTextColor(14,79,152); doc.setFontSize(10); doc.text(`ASUNTO: RECOMENDACIONES EXAMEN ${outputExamType(data.tipo_examen || '')}`, pageW/2, y+7, {align:'center',maxWidth:maxW-10}); y += 18;
      ...
      text('Según los lineamientos del programa de medicina preventiva y del trabajo de JER S.A.; se hace entrega de las recomendaciones establecidas por el proveedor de servicios de Exámenes Médico Ocupacionales, correspondientes al tipo de evaluación indicado en el encabezado.', { size: 9, after: 5 });
    ...
    async generate(documentRow, formatOverride = null, options = {}) {
      ...
      const output = {
        ...
        documentEngineVersion:SSTDocx.engineVersion || 'template-engine-v10.26'
      };
